...
export function getQuestions(): Array<DistinctQuestion> {
  ...
  const { skips } = getCzCommitConfig();

  const fields: PromptName[] = [
    ...(skips.includes('breaking') ? [] : (['isBreaking', 'breakingBody', 'breaking'] as PromptName[])),
    ...(skips.includes('issues') ? [] : (['isIssueAffected', 'issuesBody', 'issues'] as PromptName[])),
    'footer',
  ];
