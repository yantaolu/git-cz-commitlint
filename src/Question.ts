...
export default class Question {
  private _question: Readonly<DistinctQuestion & { source?: any }>;
  ...
  constructor(
    ...
    this.caseFn =
      caseFn ??
      ((input: string | string[], delimiter?: string) => (Array.isArray(input) ? input.join(delimiter) : input));
    ...
    if (enumList && Array.isArray(enumList)) {
      this._question = {
        type: multipleSelectDefaultDelimiter ? 'checkbox' : ('autocomplete' as any),
        choices: skip
          ? [
              ...enumList,
  ...
  protected validate(input: string | Choice): boolean | string {
    const output = this.filter(typeof input === 'string' ? input : input?.value);
  ...
  protected filter(input: string | string[]): string {
    ...
    if (Array.isArray(input)) {
      ...
      toCased = input.replace(new RegExp(`[^${this.multipleValueDelimiters.source}]+`, 'g'), (segment: string) => {
